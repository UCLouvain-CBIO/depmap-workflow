...
(as of 20_Q2 release) is displayed in table \@ref(tab:datatab).
...
for soft tissue cancer cell lines. The alias of `C1orf109` is Chromosome 1
Open Reading Frame 109, which produces an uncharacterized protein which may
promote cancer cell proliferation by controlling the G1 to S phase transition
[@liu2012identification]. This protein presents as an interesting candidate
target. Figure \@ref(fig:histC1orf109) displays the `crispr` data as a
histogram showing the distribution of dependency scores for gene `C1orf109`.
The red dotted line signifies the mean dependency score for that gene, while
the blue dotted line signifies the global mean dependency score for all
`crispr` measurements.
