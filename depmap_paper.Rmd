...
of common R packages such as `dplyr` [@wickham2020package] and `ggplot2` [@wickham2011ggplot2].
...
utilizes and fully depends on the `ExperimentHub` package
[pasolli2017accessible; @maintainer2020experimenthub] to store and
retrieve all versions of the Depmap data (starting from `19Q1` through `20Q1`)
in the Cloud using AWS. The `depmap` package contains accessor functions to
directly download and cache the most current datasets from the Cloud into the
local R environment. Specific datasets, such as older datasets, which have been
used in prior research can also be downloaded, if desired. This feature has the
added advantage of enhancing reproducible research, such that specific versions
of Depmap data can be selected, in addition to having access to the most current
...
dataset into the local R environment. An example is shown below:

```{r EH1, message=FALSE, warning=FALSE, echo=TRUE, eval=FALSE}
rnai <- depmap_rnai()
...
Specific datasets are downloaded and cached into the local R environment by
selecting them by their unique `EH` numbers. Shown below, datasets from the
`19_Q3` release are downloaded in this way via their such as the unique `EH`
numbers that corresponds each individual dataset.
