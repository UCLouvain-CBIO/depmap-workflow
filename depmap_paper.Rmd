...
author:
- name: Theo Killian
  affiliation: Computational Biology Unit, de Duve Institute, UCLouvain, Brussels, Belgium
- name: Laurent Gatto
  affiliation: Computational Biology Unit, de Duve Institute, UCLouvain, Brussels, Belgium
date: "`r Sys.Date()`"
...
dependencies [@broad2020depmap]. The primary method utilized in the Depmap
project to map genomic dependencies is gene knockout performed by CRISPR
[@broad2020depmap; @meyers2017computational; @dempster2019extracting; dempster2019agreement].
Genetic dependency is calculated from the observed log fold change in the amount
of shRNA detected after gene knockout [@cowley2014parallel; @mcfarland2018improved].
To correct for potential off-target effects of gene knockout in overestimating
dependency with CRISPR, the Depmap iniative utilized the CERES algorithm to
moderate the final dependency estimation [@meyers2017computational]. It should
be noted that due to advancements in the CERES algorithm to account for seed
...
[@broad2020depmap; @dempster2019extracting]. In addition genomic dependency
measurements of cancer cell lines, chemical dependencies were taken via Depmap
PRISM viability screens that as of the `20Q1` release, tested 4,518 compounds
against 578 cancer cell lines [@corsello2019non; @broad2020depmap]. The Depmap
project has also compiled additional datasets detailing molecular biological
characterization of cancer cell lines, such as genomic copy number, Reverse
Phase Protein Array (RPPA) data, TPM gene expression data for protein coding
genes and genomic mutation data. These datasets are updated quarterly on a
release schedule and are publically available under
[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) licence [@broad2020depmap].
...
package [pasolli2017accessible; @maintainer2020experimenthub] to store
and retrieve all versions of the Depmap data (starting from `19Q1`
through `20Q1`) in the Cloud using AWS. The `depmap` package contains
accessor functions to directly download and cache the most current
datasets from the Cloud into the local R environment. Specific
datasets, such as older datasets, which have been used in prior
research can also be downloaded, if desired. This feature has the
added advantage of enhancing reproducible research, such that specific
versions of Depmap data can be selected, in addition to having access
to the most current datasets. The `depmap` R package is available as
part of Bioconductor at:
...
accessor function creates an `ExperimentHub` object, which can be
queried for specific terms. The list of datasets available that
...
by selecting them by their unique `EH` numbers. Shown below, datasets
from the `19_Q3` release are downloaded in this way via their such as
the unique `EH` numbers that corresponds each individual dataset.
...
The gene `RPL14` appears several times in the top dependencies scores, and may
...
while the blue dotted line signifies the global mean dependency score for all
`crispr` measurements.
...
[@li2019changes; @hernandez2019many; @felts2019stochastic]. The extensive
coverage of the `depmap` data affords visualization of genetic expression
patterns across many major types of cancer. Displayed below is a boxplot
illustrating expression values for gene `RPL14` by lineage:
...
metadata %>%
  ...
  ggtitle("Boxplot of TPM expression values for gene RPL14 by lineage") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(legend.position = "none")
```

Differentially expressed genes and elevated genetic dependency in cancer cell
lines have been observed [@tsherniak2017defining; @mcfarland2018improved].
Therefore, such genes are may present themselves as interesting research
targets. Below is a plot of expression versus CRISPR gene dependency for
Rhabdomyosarcoma Sarcoma. The red vertical line represents the average
expression for this form of cancer, while the horizontal line represents the
average dependency for this cancer.
...
Genes with the lowest depenency scores and highest TPM gene expression are found
in the upper left section of the plot above. Such genes may present an example
as interesting research targets. Below shows a table of such genes.
...
Changes in genomic copy number also play a role in some cancer phenotypes
[@meyers2017computational; @aguirre2016genomic; @shao2019copy]. The `depmap`
data allows the display of log genomic copy number for across many cancer
lineages. Below shows such a plot for gene `RPL14` for each major type of cancer
lineage:
