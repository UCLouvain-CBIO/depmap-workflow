...
[@corsello2019non; @broad2020depmap]. As of the `20Q2` release, a new
protemic dataset was added that provides normalized quantitative profiling
of proteins by mass spectrometry [@nusinow2020quantitative]. The Depmap
...
(as of 20_Q1 release) is displayed in table \@ref(tab:datatab).
...
datatab <- dataset_list %>%
    ...
    dplyr::filter(
               Dataset %in% c("crispr_20Q2", "rnai_19Q3", "copyNumber_20Q2",
                              "TPM_20Q2", "mutationCalls_20Q2",
                              "metadata_20Q2", " RPPA_19Q3",
...
knitr::kable(datatab,
             format = "latex",
             booktabs = TRUE,
             caption = "Datasets available the depmap package. The 'Release' column indicates the most recent available release.") %>%
...
specific terms. Note that the latest `depmap` datasets are only available by
installing and running the latest `devel` version of Bioconductor. The list
of datasets available that correspond to the query, `depmap`, running 
Bioconductor 3.12 are shown below:
...
Shown below, datasets from the `20_Q1` release are downloaded in this way.

```{r load_data, message=FALSE, warning=FALSE, echo=TRUE}
## download and cache required datasets 
crispr <- eh[["EH3454"]]
copyNumber <- eh[["EH3455"]]
TPM <- eh[["EH3456"]]
