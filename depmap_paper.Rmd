...
A table of all datasets available for the depmap package (as of 20_Q1 releaste)
is displayed below \@ref(tab:table_of_datasets).
...
kable(caption = "Table of all currently available datasets for the depmap package")
```
...
cleaned by converting all datasets to the long format tibbles
[@muller2017tibble], as well as adding the unique key `depmap_id` for
all data tables, in order to make features more comparable, facilitating
